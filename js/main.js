...
// Отзывы слайдер
	...
	/* ------------------- Уроки на мобильных слайдером -------------------- */
	let initSlider = 0;

$(window).on('resize', function (e) {
	if (window.innerWidth < 601 && initSlider !== 1) {
		$('.lessons-slider_mob').slick({
			arrows: false,
			dots: true,
			slidesToShow: 1,
			slidesToScroll: 1,
		...
		initSlider = 1;
	} else if (window.innerWidth >= 601 && initSlider === 1) {
		$('.lessons-slider_mob').slick('unslick');
		initSlider = 0;
	}
}).trigger('resize');